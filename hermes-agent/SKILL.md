...
### Python environment conflicts (Windows)

If you see errors like `ModuleNotFoundError: No module named 'hermes_cli'` or `SRE module mismatch`, you likely have a Python environment conflict:

**Symptoms:**
- `hermes: command not found` even though it's installed
- `ModuleNotFoundError: No module named 'hermes_cli'`
- `AssertionError: SRE module mismatch` (Python version mismatch)
- Commands work with direct path but not via `hermes` CLI

**Cause:** Environment variables `PYTHONHOME` and `UV_INTERNAL__PYTHONHOME` point to a different Python version than where hermes-agent is installed.

**Fix:**
```bash
unset PYTHONHOME UV_INTERNAL__PYTHONHOME
hermes dashboard  # or any hermes command
```

**Alternative (specify full path):**
```bash
/c/Program Files/Python313/python -m hermes_cli.main dashboard
```
